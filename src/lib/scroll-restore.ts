...
export function normalizeSearch(search: string): string {
  ...
  // Drop volatile UI noise that shouldn't create separate scroll identities
  for (const volatile of ["photo", "quick", "_"] as const) {
 ...
 *
 * navKind:
 * - pop/forward with stored Y ÔåÆ restore
 * - push with no stored Y ÔåÆ top (intentional new entry)
 * - pop with no stored Y ÔåÆ leave scroll alone (avoid false 0 flash)
 */
...
/* ── Compatibility aliases (PathScrollRestore / rezepte) ── */

/** @deprecated Prefer buildScrollKeyNormalized — same behavior. */
export const scrollRouteKey = buildScrollKeyNormalized;

/** @deprecated Prefer readWindowScrollY. */
export const getWindowScrollY = readWindowScrollY;

/** @deprecated Prefer clearAllScrollPositions. */
export const clearScrollRestoreState = clearAllScrollPositions;

/** Read stored Y with 0 fallback (never null). */
export function readScrollPosition(key: ScrollRestoreKey | null): number {
  return peekScrollPosition(key) ?? 0;
}
